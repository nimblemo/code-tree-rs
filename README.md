...
3.  **Лексический парсинг (Regex):**
    ...
    - _Например (Rust):_ Ищет совпадения по `use_regex` (`^\s*use\s+([^;]+);`) для поиска зависимостей и по `fn_regex` для поиска функций [14].
    - _Например (PHP):_ Извлекает `namespace` и ищет зависимости как через ключевые слова `use`, `require`, `include`, так и через комментарии Composer (`// composer: package/name`) [2, 29].
...
- **Композиция `CodeInsight`:** Один `CodeInsight` _содержит_ ровно один `CodeDossier`, ровно одну структуру `CodeComplexity`, а также _множество_ (от 0 до N) `Dependency` и `InterfaceInfo` [2].
- **Композиция `InterfaceInfo`:** Каждый `InterfaceInfo` может содержать _множество_ `ParameterInfo` (аргументов) [7].
...
flowchart TD
    ...
    subgraph CodeTreeRS ["⚙️ code-tree-rs (CLI Монолит)"]
        ...
        InMemoryStore[("🗄️ In-Memory DB<br/>[Контейнер: Arc RwLock Memory]<br/><br/>Хранит промежуточные данные (ProjectStructure, CodeInsights) и метрики тайминга.")]:::database

        DiskCache[("💾 Дисковый Кэш<br/>[Контейнер: CacheManager / Файлы]<br/><br/>Хранит сериализованные результаты предыдущих запусков в .tree/cache.")]:::database
    end
...
flowchart TD
    ...
    MemStruct[("D1: In-Memory DB<br/>PROJECT_STRUCTURE")]:::store
    MemInsight[("D2: In-Memory DB<br/>CODE_INSIGHTS")]:::store
...
stateDiagram-v2
    ...
    Initializing --> ContextReady : Инициализация кэша памяти и контекста
    ...
    Profiling --> Completed : Сохранение метрик времени в Memory

    Completed --> [*] : Успешное завершение работы
```
...
stateDiagram-v2
    ...
    Pending --> ReadingCode : Получен доступ к семафору tokio

    ReadingCode --> LexicalParsing : Чтение успешно - запуск парсинга
    ReadingCode --> Skipped : Ошибка чтения или игнорирование

    state LexicalParsing {
        [*] --> Routing : LanguageProcessorManager
        Routing --> RegexMatching : Найден процессор для языка
        RegexMatching --> CodeInsightDraft : Извлечены интерфейсы и зависимости
        Routing --> Fallback : Процессор не найден - пустой черновик
    ...
    SemanticEnhancing --> StoredInMemory : Определено назначение кода
...
stateDiagram-v2
    ...
    Matched --> AssignedStrictEnum : Присвоение конкретного типа назначения
    Unmatched --> AssignedDefaultEnum : Присвоение типа Other
...
_Переменные окружения:_ Явного связывания параметров с переменными окружения (environment variables) в бизнес-логике нет. Единственное использование системного окружения — вызов `std::env::current_dir()` для определения текущей рабочей директории, если путь к проекту не передан явно [3].
...
1. **Канал CLI и Файловой системы (Ввод/Вывод):**
   - _Модуль `cli`_ получает сырой путь от пользователя (`--project-path`) и конвертирует его в инициализированный объект `Config`, считывая `.tree.toml` из файловой системы [3].
...
2. **Внутренний канал оркестрации (GeneratorContext):**
   - _Главный партнер_: In-Memory база данных (`Arc<RwLock<Memory>>`). Все агенты (например, `StructureExtractor`, `CodePurposeEnhancer`) общаются друг с другом асинхронно через этот объект [1, 6]. Агенты не передают данные напрямую друг другу — они сохраняют их в `Memory` по ключам `ScopedKeys`, откуда их могут прочитать последующие обработчики.
...
erDiagram
    ...
    CONFIG {
        ...
        bool include_tests
        bool include_hidden
        Strings_Vec excluded_dirs
        Strings_Vec excluded_files
        Strings_Vec excluded_extensions
        Strings_Vec included_extensions
        bool verbose
    }
...
_Примечание:_ Несмотря на то, что в комментариях к агенту `CodePurposeEnhancer` упоминается объединение правил с «AI analysis» (ИИ-анализом) [4], фактическая реализация вызовов к внешним API (например, OpenAI или Anthropic) в предоставленной кодовой базе и конфигурации зависимостей отсутствует [2, 3].
...
flowchart TD
    ...
    subgraph CodeTreeRS ["⚙️ code-tree-rs (CLI Монолит)"]
        ...
        MemoryDB[("🗄️ In-Memory State<br/>[Arc<RwLock<Memory>>]<br/>Хранит промежуточные структуры и метрики времени.")]:::database

        DiskCache[("💾 Локальный Кэш<br/>[CacheManager / JSON]<br/>Сброс состояния на диск для ускорения повторных запусков.")]:::database
    end
